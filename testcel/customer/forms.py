...
class EmailUserCreationForm(forms.ModelForm):
    first_name=forms.CharField(label="Votre nom et prenom")
    #last_name=forms.CharField(label="Votre prenom")

    email = forms.EmailField(label=_('Email address'))


    last_name = PhoneNumberField(widget=PhoneNumberInternationalFallbackWidget, label=_("Phone number"),help_text=("Votre téléphone au cas où nous aimerions vous joindre à propos de votre commande. Mettez, le code si vous n'êtes pas en France."),  required=True)
    ...
    class Meta:
        model = User # On se base sur le model User
        fields =("first_name","email","last_name",)
        # Clef etranger  le champs email
    ...
    def save(self, commit=True):
        ...
        if 'username' in [f.name for f in User._meta.fields]:
            user.username =user.first_name+' '+user.last_name+' ID: '+generate_username()
        if commit:
            user.save()
        return user
