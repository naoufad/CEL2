...
class Free(Base):
    ...
    """
    code = 'free-shipping'
    name = _('Retrait de votre commande à Comores En Ligne Moroni Coulé')
    description = "Le retrait des commandes s'effectue entre 10h30 à 17h00 à Comores En Ligne Moroni - Coulé, à côté de l'école Univers. Seulement pour les produits alimentaires"
...
class NoShippingRequired(Free):
    ...
    name = _('Transfert de Crédit Téléphone')
    description = "Les recharges de crédits effectués entre 7H30 à 23h00 seront disponibles après 15 min au maximum. Au delà de 23h00, le temps d'attente peut varier entre 15 min au lendemain à 7h30"
...
class FixedPrice(Base):
    ...
    name = _('Livraison de matériels de constructions: ( Bambao-Itsandra-Hamanvou )')
    description = "Nous enverrons un message au bénéficiaire lors du passage de notre coursier. La durée de livraison est de 6 jours pour le sable et 4 jours pour le reste "
    
    # Charges can be either declared by subclassing and overriding the
    # class attributes or by passing them to the constructor
    charge_excl_tax = D('10.99')
    charge_incl_tax =D('10.99')
...
class Region2(FixedPrice):
    code = 'fixed-price-shipping'
    name = _('Livraison de matériels de constructions: ( Oichili-Hambou )')
    description = "Nous enverrons un message au bénéficiaire lors du passage de notre coursier. La durée de livraison est de 7 jours pour le sable et 5 jours pour le reste"

    charge_excl_tax = D('32.99')
    charge_incl_tax =D('32.99')

class Region3(FixedPrice):
    code = 'fixed-price-shipping'
    name = _('Livraison de matériels de constructions: Hamahamet-Mitsamihuli-Mboudé-Mbadjini Ouest')
    description = "Nous enverrons un message au bénéficiaire lors du passage de notre coursier. La durée de livraison est de 7 jours pour le sable et 5 jours pour le reste"

    charge_excl_tax = D('41.75')
    charge_incl_tax =D('41.75')

class Region4(FixedPrice):
    code = 'fixed-price-shipping'
    name = _('Livraison de matériels de constructions: ( Dimani - Mbadjini EST  - Ouela et Ouzio ya Mitsamihuli )')
    description = "Nous enverrons un message au bénéficiaire lors du passage de notre coursier. La durée de livraison est de 7 jours pour le sable et 5 jours pour le reste"

    charge_excl_tax = D('57.99')
    charge_incl_tax =D('57.99')
...
class MutsamuduAnjouan_Sup(FixedPrice):
    code = 'fixed-price-shipping'
    name = _('Livraison à Domicile - Mutsamudu - Ouani')
    description = "Livré à domicile  - region Mutsamudu - Ouani en 24h ouvrées"

    charge_excl_tax = D('2.50')
    charge_incl_tax =D('2.50')
